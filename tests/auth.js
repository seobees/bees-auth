...
test('Refresh tokens become invalid on logout', async t => {
  ...
  const res = await app.post('/auth/refresh').send({ refreshToken: 'REFRESH_TOKEN_TO_DELETE_ON_LOGOUT' });
  t.is(res.status, 404);
});

test('Multiple refresh tokens are valid', async t => {
  const firstLoginResponse = await app.post('/auth/login').send({ login: 'user2', password: 'user2' });
  const secondLoginResponse = await app.post('/auth/login').send({ login: 'user2', password: 'user2' });
